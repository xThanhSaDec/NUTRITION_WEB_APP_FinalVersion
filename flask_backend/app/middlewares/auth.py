...
def require_auth(fn: Callable):
    ...
    def wrapper(*args, **kwargs):
        ...
        if not REQUIRE_JWT:
            ...
            if auth.startswith("Bearer "):
                ...
                try:
                    ...
                    if not uid:
                        # Fallback: try to decode JWT without verifying signature to extract 'sub'
                        try:
                            import base64, json
                            parts = token.split('.')
                            if len(parts) >= 2:
                                ...
                                uid = data.get('sub') or data.get('user_id') or uid
                        except Exception:
                            pass
                        if not uid:
        ...
        try:
            ...
            uid = _extract_user_id(user)
            if not uid:
                # Fallback decode in production mode too (non-fatal, no signature verify)
                try:
                    import base64, json
                    parts = token.split('.')
                    if len(parts) >= 2:
